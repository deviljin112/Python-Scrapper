...
class MultipleSearchHandler:
    ...
    def _populate_data_for_perm_roles(self):
        for role in self.list_of_roles:
            self.combined_data_dictionary.update(
                {role + "_perm": self.job_search.get_permanent_role_salary_range(role)}
            )
    ...
    def _populate_data_for_contract_roles(self):
        for role in self.list_of_roles:
            self.combined_data_dictionary.update(
                {
                    role
                    + "_contract": self.job_search.get_contract_role_day_rate_range(
                        role
                    )
                }
            )
...
if __name__ == "__main__":
    print(
        MultipleSearchHandler(
            ["sdet", "developer", "data analyst", "devops"]
        ).generate_data()
    )
