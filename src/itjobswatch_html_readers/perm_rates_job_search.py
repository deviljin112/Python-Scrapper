...
class PermRatesJobSearch:
    ...
    def _validate_search_returns_a_role(self):
        page_check = self.job_search.find(
            "td", {"class", re.compile("navContainer")}
        ).find("h1", id="h1")
        if page_check is None:
            raise ValueError(
                "Your role type led to a search try using something more specific"
            )
    ...
    def get_median_salary_excluding_london(self):
        return self.summary_table_data_selector(
...
if __name__ == "__main__":
    ...
    print(
        PermRatesJobSearch(
            "https://www.itjobswatch.co.uk/jobs/uk/sdet.do"
        ).get_summary_headers()
    )
